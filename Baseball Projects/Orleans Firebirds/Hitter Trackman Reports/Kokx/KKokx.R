Kokx <- read.csv(file.choose())
head(Kokx)

library(ggplot2)

#modifying DF
Kokx$HardHit <- Kokx$EV >= 95
Kokx$HardHit <- replace(Kokx$HardHit, Kokx$HardHit == "TRUE", "Hard Hit")
Kokx$HardHit <- replace(Kokx$HardHit, Kokx$HardHit == "FALSE", "Not Hard Hit")

#seeing frequencies
table(Kokx$HardHit)

#Hard Hit Graphs
ggplot(data = Kokx, aes(x = HardHit)) + 
  geom_bar(aes(fill = HardHit), color = "black") + 
  ggtitle("Kokx BIP Splits") + 
  theme_minimal() + 
  labs(x = "Balls in Play", y = "Count") + 
  geom_text(label = "4", y = 2, x = 1, size = 14) + 
  geom_text(label = "30", y = 15, x = 2,  size = 14) + 
  ylim(0, 35) + 
...
#dataframe
KokxHH <- subset(Kokx, EV >= 95)

#HH HeatMap
ggplot() +
  coord_fixed() +
  geom_density2d_filled(data = KokxHH, 
  ...
  geom_point(data = KokxHH, aes(x = PlateLocSide, y = PlateLocHeight, size = 2.5, alpha = .6))+
  ...
  ylim(-0.25, 4.25) +
  theme_minimal() + 
  ggtitle("Kokx Hard Hit Zone", subtitle = "Pitcher POV") +
...
KokxAVG = mean(Kokx$EV)

#test distrubution
ggplot(data = Kokx, aes(x = EV)) +
  geom_histogram(color = "black", fill = "cornflowerblue", bins =10) +
  theme_minimal() + 
  geom_vline(xintercept = KokxAVG, 
             linetype = "dashed", 
             size = 2,
             color = "red") + 
  geom_text(aes(x = KokxAVG - 5, label = "Kokx AVG", y = 5),
            color = "red", 
            angle = 90, 
            size = 6) + 
  ylab("Count") + 
  ylim(0,15) + 
  ggtitle("Kokx EV Spread") + 
  theme(plot.title = element_text(hjust = 0.5, size = 20), 
        plot.subtitle = element_text(hjust = 0.5, size = 16), 
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
