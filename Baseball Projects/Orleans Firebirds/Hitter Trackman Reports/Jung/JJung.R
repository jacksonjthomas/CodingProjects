Jung <- read.csv(file.choose())
head(Jung)

library(ggplot2)

#modifying DF
Jung$HardHit <- Jung$EV >= 95
Jung$HardHit <- replace(Jung$HardHit, Jung$HardHit == "TRUE", "Hard Hit")
Jung$HardHit <- replace(Jung$HardHit, Jung$HardHit == "FALSE", "Not Hard Hit")

#seeing frequencies
table(Jung$HardHit)

#Hard Hit Graphs
ggplot(data = Jung, aes(x = HardHit)) + 
  geom_bar(aes(fill = HardHit), color = "black") + 
  ggtitle("Jung BIP Splits") + 
  theme_minimal() + 
  labs(x = "Balls in Play", y = "Count") + 
  geom_text(label = "6", y = 3, x = 1, size = 14) + 
  geom_text(label = "11", y = 5.5, x = 2,  size = 14) + 
  ylim(0, 15) + 
...
#dataframe
JungHH <- subset(Jung, EV >= 95)

#HH HeatMap
ggplot() +
  coord_fixed() +
  geom_density2d_filled(data = JungHH, 
  ...
  geom_point(data = JungHH, aes(x = PlateLocSide, y = PlateLocHeight, size = 2.5, alpha = .6))+
  ...
  ylim(-0.25, 4.25) +
  theme_minimal() + 
  ggtitle("Jung Hard Hit Zone", subtitle = "Pitcher POV") +
...
JungAVG = mean(Jung$EV)

#test distrubution
ggplot(data = Jung, aes(x = EV)) +
  geom_histogram(color = "black", fill = "cornflowerblue", bins = 10) +
  theme_minimal() + 
  geom_vline(xintercept = JungAVG, 
             linetype = "dashed", 
             size = 2,
             color = "red") + 
  geom_text(aes(x = JungAVG - 5, label = "Jung AVG", y = 4),
            color = "red", 
            angle = 90, 
            size = 6) + 
  ylab("Count") + 
  ggtitle("Jung EV Spread") + 
  ylim(0,8) + 
  theme(plot.title = element_text(hjust = 0.5, size = 20), 
        plot.subtitle = element_text(hjust = 0.5, size = 16), 
        panel.grid.major.x = element_blank(),
