...
#modifying DF
Thomas$HardHit <- Thomas$EV >= 95
Thomas$HardHit <- replace(Thomas$HardHit, Thomas$HardHit == "TRUE", "Hard Hit")
Thomas$HardHit <- replace(Thomas$HardHit, Thomas$HardHit == "FALSE", "Not Hard Hit")

#seeing frequencies
table(Thomas$HardHit)

#Hard Hit Graphs
ggplot(data = Thomas, aes(x = HardHit)) + 
  geom_bar(aes(fill = HardHit), color = "black") + 
  ggtitle("Thomas BIP Splits") + 
  theme_minimal() + 
  labs(x = "Balls in Play", y = "Count") + 
  geom_text(label = "2", y = 1, x = 1, size = 14) + 
  geom_text(label = "25", y = 12.5, x = 2,  size = 14) + 
  ylim(0, 30) + 
...
#dataframe
ThomasHH <- subset(Thomas, EV >= 95)

#HH HeatMap
ggplot() +
  coord_fixed() +
  geom_density2d_filled(data = ThomasHH, 
  ...
  geom_point(data = ThomasHH, aes(x = PlateLocSide, y = PlateLocHeight, size = 2.5, alpha = .6))+
  ...
  ylim(-0.25, 4.25) +
  theme_minimal() + 
  ggtitle("Thomas Hard Hit Zone", subtitle = "Pitcher POV") +
...
ThomasAVG = mean(Thomas$EV)
ThomasAVG

#test distrubution
ggplot(data = Thomas, aes(x = EV)) +
  geom_histogram(color = "black", fill = "cornflowerblue", bins = 14) +
  theme_minimal() + 
  geom_vline(xintercept = ThomasAVG, 
             linetype = "dashed", 
             size = 2,
             color = "red") + 
  geom_text(aes(x = ThomasAVG - 5, label = "Thomas AVG", y = 6),
            color = "red", 
            angle = 90, 
            size = 6) + 
  ylab("Count") + 
  ylim(0,13) + 
  ggtitle("Thomas EV Spread") + 
  theme(plot.title = element_text(hjust = 0.5, size = 20), 
        plot.subtitle = element_text(hjust = 0.5, size = 16), 
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
