...
#Pitch Movement
ggplot(data = Luc, aes(x = HorzBreak, y = InducedVertBreak, color = PitchType, size = RelSpeed)) + 
...
ggplot() +
  coord_fixed() +
  geom_density2d_filled(data = Luc, 
  ...
  theme_minimal() +
  ggtitle("Luc Pitch Location", subtitle = "Pitcher POV") +
...
LucFB <-subset(Luc, PitchType == "Four-Seam")
LucCH <-subset(Luc, PitchType == "Changeup")
LucSL <-subset(Luc, PitchType == "Slider")


#FB Spin Rate
MLBfb <- 2257
LucAVGfb <- mean(LucFB$SpinRate)

ggplot(data = LucFB, aes(x = SpinRate)) +
  geom_histogram(color = "black", bins = 10, fill = "cornflowerblue") + 
  ggtitle("Luc Fastball Spin Rate Distribution") + 
  labs(x = "Spin Rate", y = "Count") +
  geom_vline(xintercept = MLBfb, 
             linetype = "dashed", 
             color = "red", 
             size = 2) + 
  geom_vline(xintercept = LucAVGfb, 
             linetype = "dashed", 
             color = "black", 
             size = 2) +
  geom_text(aes(x = MLBfb - 25, label = "MLB AVG", y = 7), 
            ...
            angle = 90, 
            size = 6) + 
  geom_text(aes(x = LucAVGfb - 25, label = "Luc AVG", y = 7), 
            color = "black", 
            angle = 90, 
            size = 6) + 
  theme_minimal() + 
...
#Slider Spin Rate
MLBslider <- 2450
LucAVGslider <- mean(LucSL$SpinRate)

ggplot(data = LucSL, aes(x = SpinRate)) +
  geom_histogram(color = "black", bins = 10, fill = "cornflowerblue") + 
  ggtitle("Luc Slider Spin Rate Distribution") + 
  labs(x = "Spin Rate", y = "Count") +
  geom_vline(xintercept = MLBslider, 
             linetype = "dashed", 
             color = "red", 
             size = 2) + 
  geom_vline(xintercept = LucAVGslider, 
             linetype = "dashed", 
             color = "black", 
             size = 2) +
  geom_text(aes(x = MLBslider - 25, label = "MLB AVG", y = 4), 
            ...
            angle = 90, 
            size = 6) + 
  geom_text(aes(x = LucAVGslider - 25, label = "Luc AVG", y = 4), 
            color = "black", 
            angle = 90, 
            size = 6) + 
  theme_minimal() + 
...
#Changeup Spin Rate
MLBchange <- 1750
LucAVGchange <- mean(LucCH$SpinRate)

ggplot(data = LucCH, aes(x = SpinRate)) +
  ...
  geom_vline(xintercept = MLBchange, 
             ...
             color = "red", 
             size = 2) + 
  geom_vline(xintercept = LucAVGchange, 
             linetype = "dashed", 
             color = "black", 
             size = 2) +
  geom_text(aes(x = MLBchange - 25, label = "MLB AVG", y = 2), 
            ...
            angle = 90, 
            size = 6) + 
  geom_text(aes(x = LucAVGchange - 25, label = "Wallerstedt AVG", y = 2), 
            color = "black", 
            angle = 90, 
            size = 6) + 
  theme_minimal() + 
...
#tunneling graph
ggplot(data = Luc, aes(x = RelSide, y = RelHeight, color = PitchType)) + 
  geom_point(size = 2.5, alpha = 0.65) + 
  xlim(0.5,4.5) + 
  ylim(3, 7) + 
  theme_minimal() + 
  labs(x = "Release Side (ft)", y = "Release Height (ft)") + 
  ggtitle("Luc Pitch Tunneling") + 
  theme(plot.title = element_text(hjust = 0.5, size = 20), 
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank(),
        axis.text.x = element_text(size = 16),
